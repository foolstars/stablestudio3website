          ...
          {header.nav?.items?.map((item, idx) => {
              ...
              return (
                <NavigationMenuLink key={idx} asChild>
                  <Link
                    ...
                    className={`flex flex-row items-center gap-2 rounded-md px-4 py-1.5 text-sm text-white transition-colors hover:bg-white/10 hover:text-white focus:bg-white/10 focus:text-white ${
            ...
            return (
              <NavigationMenuItem key={idx}>
                <NavigationMenuTrigger className="flex flex-row items-center gap-2 text-sm text-white hover:bg-white/10 hover:text-white focus:bg-white/10 focus:text-white data-[state=open]:bg-white/10 data-[state=open]:text-white">
                  {item.icon && (
            ...
            return (
              ...
              >
                {item.children && item.children.length > 0 ? (
                  <>
                    <AccordionTrigger className="flex items-center justify-between px-4 py-3 text-lg text-white hover:bg-white/10 hover:text-white data-[state=open]:bg-white/10 **:!font-normal">
                      {item.title}
                    </AccordionTrigger>
                        ...
                        {item.children?.map((subItem: NavItem, iidx) => (
                          <li key={iidx}>
                            <Link
                              href={subItem.url || ''}
                              onClick={closeMenu}
                              className="grid grid-cols-[auto_1fr] items-center gap-2.5 px-4 py-2 text-white hover:bg-white/10"
                            >
                              <div
                                aria-hidden
                  ...
                  <Link
                    href={item.url || ''}
                    onClick={closeMenu}
                    className="flex items-center justify-between px-4 py-3 text-lg text-white hover:bg-white/10 data-[state=open]:bg-white/10 **:!font-normal"
                  >
                    {item.title}
                  </Link>
                )}
  ...
  return (
    <>
      ...
      >
        <div
          className={cn(
            'absolute inset-x-0 top-0 z-50 h-18 border-transparent bg-black/45 text-white ring-1 ring-transparent backdrop-blur-sm transition-all duration-300',
            'in-data-scrolled:border-white/10 in-data-scrolled:bg-black/55 in-data-scrolled:border-b in-data-scrolled:backdrop-blur',
            'has-data-[state=open]:ring-white/10 has-data-[state=open]:bg-black/75 has-data-[state=open]:h-[calc(var(--navigation-menu-viewport-height)+3.4rem)] has-data-[state=open]:border-b has-data-[state=open]:shadow-lg has-data-[state=open]:shadow-black/10 has-data-[state=open]:backdrop-blur',
            'max-lg:in-data-[state=active]:bg-black/80 max-lg:h-14 max-lg:overflow-hidden max-lg:border-b max-lg:in-data-[state=active]:h-screen max-lg:in-data-[state=active]:backdrop-blur'
          )}
        >
